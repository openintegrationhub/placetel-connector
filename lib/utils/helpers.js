...
async function deleteContact(msg, cfg) {
  try {
    if (!msg.metadata || !msg.metadata.recordUid) {
      console.error('Supposed to delete but was not supplied a recordUid');
      return false;
    }
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      while (nextPage && page < 20) {
        ...
        let currentContacts = response.body;

        if (Array.isArray(currentContacts)) {
          currentContacts = currentContacts.filter(contact => new Date(contact.updated_at).getTime() > snapshot.lastUpdated);
...
module.exports = {
  upsertContact, getContacts, deleteContact,
};
